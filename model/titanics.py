...
class TitanicRegression:
    ...
    def initTitanic(self):
        ...
        self.td.dropna(
            inplace=True
        )  # drop rows with at least one missing value, after dropping unuseful columns
    ...
    def predictSurvival(self, passenger):
        ...
        cols = ["embarked_" + val for val in self.enc.categories_[0]]
        new_passenger.update(
            {col: value for col, value in zip(cols, embarked_encoded.toarray()[0])}
        )
        ...
        aliveProb = self.logreg.predict_proba(self=self, x=new_passenger_df)[:, 1]
